...
trait IdUtil[IId_t[_]] {
  ...
  /** Приведение коллекции инстансов к коллекции id'шников.
    ...
    */
  def els2ids[Id_t](els: IterableOnce[IId_t[Id_t]]): Iterator[Id_t]

  /** Приведение коллекции инстансов ко множеству id'шников. */
  def els2idsSet[Id_t](els: IterableOnce[IId_t[Id_t]]): Set[Id_t] = {
    els2ids(els)
      .toSet
  }
  ...
  def els2idMapIter[Id_t, T <: IId_t[Id_t]](els: IterableOnce[T]): Iterator[(Id_t, T)]
  ...
  def elsFut2idMapFut[Id_t, T <: IId_t[Id_t]](elsFut: Future[IterableOnce[T]])
...
object IId extends IdUtil[IId] {
  ...
  override def els2ids[Id_t](els: IterableOnce[IId[Id_t]]): Iterator[Id_t] = {
    els
      .iterator
      .map(_.id)
  }
  ...
  /** Приведение списка элеменов в итератору, пригодному к дальнейшему конвертацию в карту. */
  override def els2idMapIter[Id_t, T <: IId[Id_t]](els: IterableOnce[T]): Iterator[(Id_t, T)] = {
    if (els.isEmpty) {
      Iterator.empty
    } else {
      els
