Browser=Браузер
I=Я
Adv=Размещение
When=Когда
Sum=Сумма
Reason=Причина
All=Все
Add=Добавить
Companies=Компании
Company=Компания
Name=Название
Address=Адрес
Delete=Удалить
Deletion=Удаление
Owner=Владелец
Mart=Торговый центр
Marts=Торговые центы
Address=Адрес
Create=Создать
Created=Создан
Edit=Редактировать
Shop=Магазин
Shops=Магазины
Save=Сохранить
Saving=Сохранение
Saved=Сохранено
Continue=Продолжить
Next=Далее
Back=Назад
Site=Сайт
Floor.number=Номер этажа
Section.number=Номер секции
Close=Закрыть
Your=Ваш
Preview=Предпросмотр
Install=Установить
Uninstall=Деинсталлировать
Town=Город
Phone=Телефон
tel.=тел.
Photo=Фото
Node.photos=Фотографии магазина
Node=Узел
Welcome.screen=Экран приветствия
Logo=Лого
Other=Другой
Others=Другие
Privacy.policy=Политика приватности
Status=Статус
Error=Ошибка
Unknown.error=Неизвестная ошибка
Page.link=Ссылка на страницу
Problem.occured=Возникла проблема
without.confirmation=без подтверждения
Cancel=Отмена
Description=Описание
Height=Высота
Width=Ширина
Profile=Профиль
Billing=Биллинг
Events=События
Logout=Выход
Help=Помощь
Logout.suggest.io=Покинуть Suggest.io?
Balance=Баланс
Info.about.prods.and.svcs=Информация о товарах и услугах
Date=Дата
Amount._money=Сумма
Bill.id=Номер
Bill.details=Основание
Show.more=Показать ещё
Show.all=Показать всё
Show.details=Показать подробности
Show=Показать
No.records=Нет записей
Nothing.found=Ничего не найдено
Provider=Провайдер
Page=Страница
Signup.confirmation=Подтверждение регистрации
Signup.finished=Регистрация завершена.
Predicate=Предикат
Edited=Редактировалось
Currency=Валюта
Refuse=Отказать
Approve=Одобрить
Delete.all=Удалить все
Something.gone.wrong=Что-то пошло не так
Center=Центр
Please.wait=Пожалуйста, подождите...
Today=Сегодня
Any=Любой
_N=№
in=в
near=около
Please.try.again=Попробуйте ещё раз
Please.try.again.later=Пожалуйста, повторите попытку позже.
Demo=Демо
Server.request.in.progress.wait=Происходит запрос к серверу, подождите...
Is.enabled=Включён?
Are.you.sure=Вы уверены?
This.action.cannot.be.undone=Это действие нельзя будет отменить.
Yes.delete.it=Да, удаляйте
Change=Изменить
Read.more=Читать далее...
Describe.problem=Опишите проблему
Rotation=Вращение
Clear=Очистить
Reload=Перезагрузить
Geolocation=Геолокация
Distance=Расстояние

requisites=Платёжные реквизиты для вывода средств

Type.password=Введите пароль
Retype.password=Повторите пароль
Password.change=Смена пароля
Password.recovery=Восстановление пароля
Type.current.password=Введите текущий пароль
Password.reset=Сброс пароля
Type.new.password=Введите новый пароль
Confirm.new.password=Подтвердите новый пароль
Confirm.password=Подтвердите пароль
New.password=Новый пароль
New.password.saved=Новый пароль сохранен.
Invalid.password=Неправильный пароль
Inacceptable.password.format=Недопустимый формат пароля

Color=Цвет
Colors=Цвета
Manufacturer=Производитель
Size=Размер
Sizes=Размеры
Price=Цена
Information=Информация
Write=Написать
WRITE=НАПИСАТЬ

Offer=Оферта
Showcase=Выдача

slaves.of.m=АРЕНДАТОРЫ
slaves.of.R=РЕСТОРАНЫ

add.to.other.s=Добавить в другой магазин
add.to.other.r=Добавить в другой ресторан
add.to.other.m=Добавить в другой торговый центр
add.to.other.R=Добавить в другую сеть ресторанов

amt.of.type.s=Магазин
amt.of.type.m=Торговый центр
amt.of.type.r=Ресторан
amt.of.type.R=Супервайзер сети ресторанов
amt.of.type.a=Транспорт
amt.of.type.b=Станция метро
amt.of.type.c=Район города
amt.of.type.d=Город
amt.of.type.e=Спорт
amt.of.type.f=Округ мегаполиса
amt.of.type.g=Другое здание

amts.of.type.s=Магазины
amts.of.type.m=Торговые центры
amts.of.type.r=Рестораны
amts.of.type.R=Сети ресторанов
amts.of.type.a=Транспорт
amts.of.type.b=Станции метро
amts.of.type.c=Городские районы
amts.of.type.d=Города
amts.of.type.e=Спорт
amts.of.type.f=Округа мегаполисов
amts.of.type.g=Другие здания

editing.of.amt.profile.s=Редактирование профайла магазина
editing.of.amt.profile.m=Редактирование профайла ТЦ
editing.of.amt.profile.r=Редактирование профайла ресторана
editing.of.amt.profile.R=Редактирование профайла супервайзера
editing.of.amt.profile.a=Редактирование профайла транспортного узла
editing.of.amt.profile.b=Редактирование профайла станции метро

District=Район
Districts=Районы
Area=Округ
Areas=Округа

adv.period.custom=Задать вручную
adv.period.P3D=Три дня
adv.period.P1W=Неделя
adv.period.P1M=Месяц

_x.asking.for.adv=запрашивает размещение.

ngl.bu=Здание
ngl.td=Район города
ngl.to=Город

ngls.bu=Здания
ngls.td=Районы городов
ngls.to=Города

bf.text.example=Текст {0}

Input.text.from.picture=Введите текст с картинки
error.captcha=Неверный код проверки.

blocks.field.isWide=Использовать широкоформатное отображение?
Title=Заголовок
blocks.field.bgColor=Цвет фона
blocks.field.href=Ссылка на внешний ресурс
Add.text.block=Добавить текстовый блок
N.modules={0} модулей

ad.stat.action.v=Просмотр
ad.stat.action.c=Клик

mir.type.a=Рекламодатель
mir.type.w=Владелец Wi-Fi

adv.mode.o=Принятое
adv.mode.r=Запрошенное
adv.mode.e=Отклонённое

join.qsn.haveWifi=У вас есть Wi-Fi?
join.qsn.fullCoverage=Wi-fi оборудование покрывает всю необходимую площадь помещения?
join.qsn.knownEquipment=Вы знаете какое wi-fi оборудование у вас используется?
join.qsn.altFw=Оборудование работает на альтернативной прошивке?
join.qsn.isWrtFw=Оборудование работает на прошивке OpenWRT, DD-WRT или RouterOS?
join.qsn.landlineInet=У вас стационарный интернет?
join.qsn.smallRoom=У меня небольшое помещение без сплошных бетонных перегородок.
join.qsn.audienceSz=Предполагаемое количество одновременно подключаемых устройств?
join.qsn.audienceSz.lt20=≤ 20
join.qsn.audienceSz.lt50=≤ 50
join.qsn.audienceSz.gt50=> 50
Request.invite=Запросить инвайт
Audience.descr=Описание аудитории
Daily.people.traffic=Суточный трафик
N.humans.per.day={0} человек в сутки.
Office.phone.number=Офисный телефон
Your.IR.accepted=Ваш запрос на подключение к системе принят.

Show._ad=Выводить

nth.floor={0}-й этаж

username.unknown=Кто здесь?

error.gallery.too.large=Слишком много картинок в галлерее.
error.no.money=Недостаточно средств
error.invalid=Неверное значение
error.invalid.java.util.Date=Неверный формат даты
error.required=Поле обязательно для заполнения
error.number=Введите число
error.real=Введите число
error.real.precision=Real number value with no more than {0} digit(s) including {1} decimal(s) expected
error.min=Значение должно быть больше или равно {0}
error.min.strict=Значение должно быть больше {0}
error.max=Значение должно быть меньше, либо равно {0}
error.max.strict=Значение должно быть меньше {0}
error.minLength=Минимум {0} символов
error.maxLength=Максимальная длина — {0} символов
error.unknown.email_pw=Неверные email/пароль.
error.contract.not.agreed=Пожалуйста, внимательно прочитайте текст договора. В случае согласия со всеми условиями договора, подтвердите своё согласие, поставив галочку ниже.

constraint.required=Обязательно
constraint.min=Минимальное значение: {0}
constraint.max=Максимальное значение: {0}
constraint.minLength=Минимальная длина: {0}
constraint.maxLength=Максимальная длина: {0}
constraint.email=Email

im.op.a=Кроп абс
im.op.b=Гравитация
im.op.c=Ресайз абс
im.op.d=Интерласинг
im.op.e=Размывка по Гауссу
im.op.f=Качество сжатия
im.op.g=Экстент
im.op.h=Стрип метаданных
im.op.i=Фильтр
im.op.j=Сэмплинг
im.op.k=Кроп процентный


stuff.menu.ABOUT=О ПРОЕКТЕ
stuff.menu.HELP=ПОМОЩЬ
stuff.menu.LIVE=ВЖИВУЮ
stuff.menu.CONTACTS=КОНТАКТЫ


ai.mad.rrr.descr.scalasti=Обновление значений текстовых полей целевой рекламной карточки с помощью шаблонизатора ScalaSti и шаблонной карточки с #-разделителями.
ai.ch.descr.gidromet.rss=Парсер прогнозов погоды росгидрометцентра в формате RSS (meteoinfo.ru/rss/forecasts/26063).
ai.ch.descr.cbrf.xml=Парсер валютных котировок XML API с сайта ЦБ РФ (cbr.ru/scripts/XML_daily.asp).


live_search_for_your_website=Живой поиск для вашего сайта
site_indexation_caption_initial=ЗАПУСК ИНДЕКСАЦИИ
site_indexation_caption=ИНДЕКСАЦИЯ САЙТА
All.rights.reserved=Все права защищены

locale=ru_RU
locale.momentjs=ru

about_link=О ПРОЕКТЕ
feedback_link=КОНТАКТЫ

undefined_event=событие не определено
crawler_activity_label=Активность кравлера
indexation_in_progress=сайт индексируется
indexation_complete=сайт проиндексирован, режим мониторинга
nxdomain=ошибка, домен не существует

too_complex_content_or_overloaded_system=ошибка: указанная ссылка вернула слишком ресурсоемкий контент или сервер перегружен... Попробуйте позже
unexpected_response_content=ошибка: указанная ссылка не ведет на html-страницу
econnrefused=ошибка соединения (econnrefused): не удалось установить соединение с сервером. Попробуйте добавить сайт еще раз.
econnreset=ошибка соединения (econnreset): не удалось установить соединение с сервером. Попробуйте добавить сайт еще раз.
econnaborted=ошибка соединения (econnaborted): не удалось установить соединение с сервером. Попробуйте добавить сайт еще раз.
crawler_error=Ошибка кравлера
indexation_error=Ход  индексации
page_deduplicated=Страница дедуплицирована
page_crawled=Страница проиндексирована
url_crawled=Документ пройден индексатором

average_response_time=Среднее время ответа сервера
stat_10min=За последние 10 минут пройдено
stat_hour=За последний час пройдено страниц
stat_day=За последний день пройдено страниц
stat_week=За последнюю неделю пройдено страниц
stat_month=За последний месяц пройдено страниц
stat_year=За последний год пройдено страниц

email_us_at=Напишите нам на адрес
or_fill_the_form=или заполните форму ниже

Your.name=Ваше имя
your_message=Ваше сообщение
about_step_one=Мы — Suggest.io. Мы делаем лучший пользовательский поиск.
about_step_two=Для начала работы с системой достаточно ввести адрес вашего сайта. Система автоматически начнет индексацию.
about_step_three=Скопируйте код из открывшегося<br/> окна и добавьте его во все шаблоны страниц вашего сайта перед закрывающим тэгом </body>
unknown_character_encoding=Невозможно определить кодировку
message_sent=Ваше сообщение успешно доставлено команде Suggest.io!
install_success=Подключение к Suggest.io. завершено. Живой поиск на вашем сайте уже работает.
http_500=Внутренняя ошибка сервера<br/>(HTTP 500)

hostname_prohibited=Сайт запрещен для индексации

zuck=Марк, ты?
durov=Павел, ты?"

invalid_url=Указан неверный URL

Yes=Да
No=Нет

please=Пожалуйста

reindex_launched=Процесс индексации запущен
reindex_try_later=Превышен лимит. Повторите попытку позже.
saved_successfully=Изменения сохранены
img_site_preferences=Как настроить сайт?
overloaded_message=Сервис временно перегружен. Повторите попытку позднее.

copy_js_code_to_your_site=Скопируйте JavaScript код и добавьте его к себе на сайт
custom_search_solutions_br=разработка<br/>индивидуальных<br/>поисковых<br/>решений.

blog=Blog
help=Помощь

and=и
or=или

s.bb.title=Настройка браузера

mappers.url.invalid_url=Некорректный URL.
mappers.url.only_http_https_allowed=Недопустимый протокол. Только HTTP и HTTPS разрешены.
mappers.url.hostname_prohibited=Этот сайт нельзя проиндексировать.


f.feedback_sent_success=Спасибо! Ваше сообщение отправлено разработчикам Suggest.io.
Send=Отправить


c.s.search_query.empty=Пустая строка поиска. Что-то ищем?

tariff.type.p=Абонплата


weather.sky.state.Clear=Ясно
weather.sky.state.PartlyCloudy=Местами облачно
weather.sky.state.Cloudy=Облачно
weather.sky.state.VaryCloudy=Переменная облачность

wind.direction.north=северный
wind.direction.south=южный
wind.direction.east=восточный
wind.direction.west=западный
wind.direction.north-west=северо-западный
wind.direction.north-east=северо-восточный
wind.direction.south-west=юго-западный
wind.direction.south-east=юго-восточный


VKontakte=ВКонтакте

I.at.vk=Я ВКонтакте
I.at.fb=Мой Facebook
I.at.tw=Мой Твиттер
I.at.gu=Я на ГосУслугах

GovServices.ESIA=Гос.услуги (ЕСИА)
ESIA._unabbrevated=Единая система идентификации и аутентификации

error.service.unknown=Этот сайт не поддерживается.
error.required.at.least.one.adv.service=Необходимо выбрать хотя бы один сервис из списка.
e.js.script.load.failed=Не удалось загрузить {0}. Возник таймаут соединения ({1} мс).
error.adv.ext.s2s.img.upload.refused=Сервер {0} отказал в загрузке картинки.
error.adv.ext.s2s.img.upload.failed=Не удалось установить соединение с сервером {0}. Нельзя загрузить картинку.
error.sio.internal=Внутренняя ошибка suggest.io. {0} {1}
error.adv.ext.js.refused=Размещение прервано или отменено.

e.event.not.found=Событие не найдено.

reg.emailpw.email.subj=Подтверждение регистрации в сети Suggest.io

adv.ext.ret.sc=Всю выдачу
adv.ext.ret.sc.hint=Ссылка на выдачу вашего магазина будет размещена вместе с текущей карточкой.
adv.ext.ret.ad=Только эту карточку
adv.ext.ret.ad.hint=Ссылка на текущую карточку будет размещена вместе с текущей карточкой.

e.ext.adv.unathorized=Пользователь не авторизован.
e.ext.adv.permissions=Недостаточно прав.
e.ext.adv.permissions.group=Недостаточно прав для постинга на странице "{0}".
e.ext.adv.target.url.invalid=Некорректный URL цели.
e.adv.ext.api.login=Возникла ошибка в процедуре логина.
e.adv.ext.api.init=Не удалось инициализировать клиент.
e.adv.ext.api=Ошибка взаимодействия с сервисом.
e.adv.ext.adp.internal=Внутренняя ошибка в клиенте {1}.
e.adv.ext.dom.update=Не удаётся изменить содержимое текущей страницы.
e.url.load.failed=Таймаут загрузки URL "{0}". Истекло время ожидания ({1} мс).
e.adv.ext.your.browser.block.popups=Ваш браузер блокирует всплывающие окна. Вам необходимо разрешить все всплывающие окна для suggest.io и повторить попытку.
e.adv.ext.unknown.tg.url=Suggest.io не понимает указанную ссылку. Пожалуйста, проверьте правильность ссылки на целевую страницу.
New.target=Новая цель
in.progress=в процессе...
status.success=успешно
Show.tech.error.info=Показать технические подробности ошибки
e.adv.ext.service.startup.error=Ошибка запуска сервиса
Following.targets.skipped=Следующие цели были пропущены
Adv.on.ext.services.running=Происходит размещение рекламной карточки на внешних сервисах
Adv.ext=Внешнее размещение карточки
Add.target=Добавить цель
Adv.deploy.now=Начать размещение
adv.ext.tg.name.hint=Произвольное название цели. Отображается только владельцу(-ам) данного магазина.
adv.ext.tg.url.hint=Ссылка на страницу на целевом сайте. Например, ссылка на ваш профиль/страницу/группу/... в соц.сети ВКонтакте или Facebook или иную цель, поддерживаемую системой Suggest.io.

Adv.deploy.in.process=Идёт размещение рекламной карточки
Do.not.close.current.page.until.deploy.finish=Не закрывайте эту вкладку до завершения процесса

Adv.in.ext.services=Разместить в соц.сети
Adv.in.geo.node=Разместить в геолокации
Adv.history=История размещений
Ads.were.adv=Рекламные карточки отправлены на размещение.
Adv.reqs.sent=Запросы на размещение отправлены.
Adv.req.refused=Размещение отклонено.
Adv.req.accepted=Реклама будет размещена.

Create.ad=Создать{0}карточку
Ad.cards=Рекламные карточки
You.can.look.on.ads.in=Вы можете посмотреть, как выглядят ваши к карточки в
_ad.showcase.link=рекламной выдаче

Ads.all=Все карточки
Ads.on.review=На рассмотрении
Ads.published=Размещенные

Click.to.see.information=Нажмите для просмотра информации
Adv.accepted=Принята к размещению
Adv.right.now=Размещена сейчас

ad.Manage=Управление

passwords.do.not.match=Пароли не совпадают. Попробуйте снова.

Checking.your.browser=Тестируем ваш браузер...


Ad.card.edit=Редактирование рекламной карточки
Ad.card.create=Создание рекламной карточки
Ad.card.will.be.deleted=Рекламная карточка будет удалена.
Ad.deleted=Рекламная карточкам удалена.
Choose.category=Выбор категории
Ad.layout=Макет рекламной карточки
Ad.edit.pattern=Паттерн
Ad.edit.pattern.color=Цвет паттерна
Bg.color=Цвет фона
Upload.file=Загрузить{0}файл
Crop=Кадрировать
Choose.color=Выберите цвет
Fill.by.single.color=Залейте однотонным цветом
Ad.large.bg.img.info=Обратите внимание, что на большинстве устройств в выдаче карточки могут увеличиваться в размерах вдвое. Поэтому, мы советуем выбирать изображения, как минимум вдвое больших размеров, чем размер блока.

Disable.reason=Причина отключения

Switch.node=Сменить узел
New.shop.created.fill.info=Создан новый магазин. Пожалуйста, заполните необходимые поля.
Ad.cards=Рекламные карточки

My.ad.net=Моя рекламная сеть
Choose.node=Выберите магазин
Create.node=Создать магазин
You.have.no.nodes.create.first=У вас пока нет ни одного магазина. Вы можете создать ваш первый магазин прямо сейчас.
Node.name=Название магазина
Node.name.hint=Название можно изменить в любое время.

Main.color=Основной{0}цвет
Bg.color.of.sc.hint=Цвет фона и шапки, где будет располагаться ваш логотип
Content.color=Цвет{0}контента
Fg.color.of.sc.hint=Цвет текста, иконок и кнопок
Text.color=Цвет текста
Edit.profile=Редактировать профиль
Advertisers=Рекламодатели

Info.about.D0.choosing.color.solution=Информация о выборе{0}цветового решения

Upload.logo=Загрузить{0} логотип
Upload.photo=Загрузить{0} фотографию
Upload.image=Загрузить{0} изображение
Add.photo=Добавить{0} фотографию
Welcome.bg.hint=Фон приветствия, фоновая картинка, на ней будет располагаться логотип или знак

Color.of.text.and.icons=Цвет текста{0} и иконок
Color.of.bg.and.caption=Цвет фона{0} и шапки
Use.contrast.colors.hint=Для более высокой читабельности вашего контента{0} выбирайте цвета контрастные друг от друга.

Pay=Оплатить
Payment=Платеж
Payment.comment=Комментарий к платежу
Bill.account.and.tariffs=Счет и тарифные планы
Transactions.history=История операций
Transactions=Транзакции
No.transactions.found=Транзакций не найдено.
Add.funds=Пополнить баланс
Total.funds=Всего на счете
Available.funds=Доступно для операций
Blocked=Заблокировано
Tariff.rate.of.0=Ставка тарифных планов {0}
My.node.tariff.rate=Ставка тарифных планов моего магазина
Adv.tariff.rates=Ставки тарифных планов рекламного размещения
Holidays.primetime=Праздники/праймтайм
Week.days=Будние дни
Weekend=Выходные дни
All.week=Вся неделя
Minimal.module=Минимальный модуль
scheme.left=на схеме слева
Agreement.btw.CBCA.and.node.tariffs.for.year=Cоглашением между компанией CBCA и {0} "{1}" определены тарифные планы на {2} год.

Adv.tariff=Тариф размещения
Inherited=Унаследован
Set.manually=Выставлен вручную
_per_.day=/день
Comission.0.pct.for.sio=Комиссия {0}% в пользу Suggest.io.

day24h=сутки

New.events.cnt={0} новых событий
User.profile.edit=Редактирование профиля пользователя

Under.consideration=На рассмотрении
This.page.contains.incoming.adv.reqs=В этом разделе появляются запросы на размещение рекламных карточек.
Adv.rates.wrote.in.CBCA.contract=Расценки на размещение прописаны в договоре с компанией CBCA.
Click.on.card.for.info.consider=Нажмите на карточку для детального просмотра и утверждения, либо отклонения.
Click.for.more.info=Нажмите для{0} просмотра{0} информации

Ad.adv.history=История размещений рекламной карточки
Geo.adv=Размещение рекламы в геолокации
Send.request=Отправить запрос
Total.amount._money=Общий счет
No.history=История отсутствует
Denied=Отказано
Approved._adv=Размещено
Adv.request.from=Запрос размещения рекламы от
Adv.in.cats=Размещение в категориях товаров и услуг
Adv.on.main.screen=Размещение на главном экране
Write.refuse.reason=Напишите причину отказа
Approve._adv=Разместить
Send.rejection=Отправить отказ
Cancel.adv=Отменить размещение
Select.town=Выберите город
All.places=Все места
Main.screen=Главный экран
_adv.Online.now=Отображается сейчас
List=Список
advertised=размещено
Date.choosing=Выбор даты
Advertising.period=Срок размещения
Date.start=Дата начала
Date.end=Дата окончания
Your.ad.will.adv=Ваша карточка будет размещена
from._date=с
From._date=С
till._date=по
refused.adv=отказал в размещении
approved.adv=одобрил размещение

Adv.on.map.hint=Если включено, то на карте будет отображаться круг. Пользователи, находящиеся внутри круга, увидят вашу рекламную карточку.

Using.add.shop.btn.you.can=Используя кнопку "Добавить магазин", вы всегда можете создать новый магазин с новой выдачей, дизайном и карточками.
Start.your.work.with.ads.mgr=Начните свою работу с простого и удобного менеджера рекламных карточек. С его помощью, Вы может оперативно создавать и изменять ваши рекламные карточки, размещать их на других узлах рекламной сети Suggest.io и даже на других сайтах, таких как популярные социальные ВКонтакте и Facebook.
Hide.event=Скрыть событие
Geotargeted.ad=Геотаргетированная реклама
Advertise=Разместить
How.it.works=Как это работает

Select.your.lang=Выберите свой язык
__This.lang.name__=Русский
Switch.lang=Сменить язык

Login.page.title=Вход
Login=Войти
Login.to.sio=Вход в Suggest.io
Remember.me=Запомнить меня
Forgot.password=Забыли пароль?
Recover.password=Восстановить пароль?
Password.recovery=Восстановление пароля
Recovery.key.expired=Ключ восстановления истёк
or.used.cannot.recover.pw=или уже использован. Невозможно продолжить восстановление пароля.
Sign.up=Регистрация
Password=Пароль
Go.back=Назад
Epw.reg.msg.sent=Письмо с подтверждением регистрации было отправлено на указанный email.
Epw.follow.msg.link.to.finish=Пройдите по ссылке в письме для завершения регистрации.
Sign.up.to=Регистрация в {0}
Hello=Здравствуйте
Msg.sent.auto.noreply=Сообщение отправлено автоматически. Отвечать на него не требуется.
Your.reg.in.sio.confirmed=Ваша регистрация в системе геонацеленной мобильной рекламы {1} одобрена.
Follow.this.link.to.complete.epw.reg=Проследуйте по {0}этой ссылке{1} для завершения процедуры регистрации.
If.link.not.open.cp.link.to.browser=Если ссылка не открывается, вставьте в строку браузера:
If.you.not.epw.reg.delete.msg=Если вы не регистрировались в {0}, просто удалите данное сообщение.
Password.recovery.in=Восстановление пароля в {0}
Sys.rcvd.pw.recover.request=Система геонацеленной мобильной рекламы {0} получила запрос на восстановление пароля к вашей учетной записи:
To.recover.password.use.this.link=Для восстановления пароля перейдите по {0}ссылке{1}.

Type=Тип

To.addr.msg.sent.with.recover.link=На адрес {0} будет отправлено письмо,{1} содержащее ссылку для восстановления пароля.
Check.your.mailbox=Проверьте ваш почтовый ящик.
Email.used.during.registration=Электронная почта, указанная при регистрации
Your.identity.accepted.saved=Ваши данные приняты и сохранены
In.seconds.you.ll.rdr.to.your.shop=Через несколько секунд вас перебросит в ваш магазин.
If.no.rdr.click.here=Если перенаправление не произошло, нажмите {0}здесь{1}.
Mailbox.ll.be.mailed.with.reg.confirm.msg=На электронную почту будет отправлено письмо с подтвержением регистрации.
Getting.started.with.sio.you.agreed.with.policy=Приступая к работе с Suggest.io, Вы соглашаетесь с {0}Политикой конфиденциальности{1}.
Your.email.addr=Ваш адрес email

Clothing=Одежда
Footwear=Обувь
Tourism.and.recreation=Туризм и отдых
Electronics=Электроника
Sporting.goods=Спорттовары
Cosmetics=Косметика
Homeware=Товары для дома
Cafes.and.restaurants=Кафе и рестораны
Gastronomy=Гастрономия
Info=Инфо
Fitness=Фитнесс
Services=Услуги
Theatres=Театры
Sport=Спорт
Spa.and.massage=Спа и массаж
Realty=Недвижимость
Museums.and.exhibitions=Музеи и выставки
Cinema=Кино
Banking=Банки

About.sio=О проекте
Suggest.io.Project=Проект Suggest.io
About.sio.node.id=AU1MCCmy2A6B8K1LbX6u
Suggest.io._transcription=[Саджэ́стио]
Transcription=Транскрипция
Detect.geolocation=Определить местоположение

i=я
suggest=предлагаю
isuggest=япредлагаю

Please.close.this.wnd.if.not.closed.auto=Пожалуйста, закройте это окно, если оно не закрылось автоматически.

e.sz.mult.too.low=Слишком маленький коэффициент масштабирования.
e.sz.mult.too.high=Слишком большой коэффициент масштабирования.

Image=Изображение
Background=Фон
Foreground=Передний план

GeoTag=Гео-тег
Tags=Теги
Tag=Тег
Tags.choosing=Выбор тегов
e.tag.len.min=Слишком короткий тег.
e.tag.len.max=Слишком длинный тег.
Add.tags=Добавить теги

Overwrite.all=Перезаписать все

Ad.created=Рекламная карточка создана.
Changes.saved=Изменения сохранены.
No.changes=Без изменений.

Your.msg.sent=Ваше сообщение отправлено.
Your.req.sent=Ваш запрос отправлен.
Activation.impossible=Активация невозможна.

Ntype.p=Человек
Ntype.n=Кабинет
Ntype.a=Рекламная карточка
Ntype.t=Тег
Ntype.m=Медиа
Ntype.i=Картинка
Ntype.b=Bluetooth-маячок
Ntype.mf=Файл
Ntype.e=Внешние ресурсы
Ntype.ev=Видео
Ntype.er=Ресурс

Ntypes.p=Люди
Ntypes.n=Кабинеты
Ntypes.a=Рекламные карточки
Ntypes.t=Теги
Ntypes.m=Медия
Ntypes.i=Картинки
Ntypes.b=Маячки Bluetooth
Ntypes.mf=Файлы
Ntypes.e=Внешние ресурсы
Ntypes.ev=Видео
Ntypes.er=Ресурсы
Nodes=Узлы

edge.predicate.a=Владелец
edge.predicate.c=Adv mdr dg
edge.predicate.d=Создатель
edge.predicate.e=Логотип
edge.predicate.f=Геородитель
edge.predicate.g=Модерировал
edge.predicate.h=Логотип приветствия
edge.predicate.i=Приложение
edge.predicate.iF=Из файла
edge.predicate.iD=Сервис-дистрибьютор
edge.predicate.b=Блоб
edge.predicate.j=Файл
edge.predicate.bs=Кусок данных
edge.predicate.k=Ресивер
edge.predicate.ks=Размещение на продьюсере
edge.predicate.ka=Размещение в узле
edge.predicate.l=Тег
edge.predicate.lg=Размещение в геотеге
edge.predicate.ls=Сам себе тег
edge.predicate.ld=Прямой теггинг
edge.predicate.m=Фон
edge.predicate.n=Локация узла
edge.predicate.np=Гео-размещение узла
edge.predicate.o=Гео-размещение рекламной карточки
edge.predicate.p=Находится внутри
edge.predicate.r=Карта рекламодателей
edge.predicate.s=jd-контент
edge.predicate.st=Текстовый контент
edge.predicate.si=Картинка-контент
edge.predicate.sv=Видео-контент
edge.predicate.q=Идент
edge.predicate.qe=Электронная почта
edge.predicate.qp=Номер телефона
edge.predicate.qi=Идентификатор
edge.predicate.qw=Пароль
edge.predicate.u=Короткая ссылка

Your.name=Ваше имя
First.name=Имя
Last.name=Фамилия

Type.digits.from.picture=Введите цифры с картинки.

To.adv.on.map=Разместить на карте
Adv.on.map=Размещение на карте
No.enought.deposit=Недостаточно средств на счете
City.place.choose=Выбор города/ места

Month.N.1=Январь
Month.N.2=Февраль
Month.N.3=Март
Month.N.4=Апрель
Month.N.5=Май
Month.N.6=Июнь
Month.N.7=Июль
Month.N.8=Август
Month.N.9=Сентябрь
Month.N.10=Октябрь
Month.N.11=Ноябрь
Month.N.12=Декабрь

ofMonth.N.1=января
ofMonth.N.2=февраля
ofMonth.N.3=марта
ofMonth.N.4=апреля
ofMonth.N.5=мая
ofMonth.N.6=июня
ofMonth.N.7=июля
ofMonth.N.8=августа
ofMonth.N.9=сентября
ofMonth.N.10=октября
ofMonth.N.11=ноября
ofMonth.N.12=декабря


DayOfW.N.1=Пн
DayOfW.N.2=Вт
DayOfW.N.3=Ср
DayOfW.N.4=Чт
DayOfW.N.5=Пт
DayOfW.N.6=Сб
DayOfW.N.7=Вс

dayOfW.N.1=пн
dayOfW.N.2=вт
dayOfW.N.3=ср
dayOfW.N.4=чт
dayOfW.N.5=пт
dayOfW.N.6=сб
dayOfW.N.7=вс


DayOfWeek.N.1=Понедельник
DayOfWeek.N.2=Вторник
DayOfWeek.N.3=Среда
DayOfWeek.N.4=Четверг
DayOfWeek.N.5=Пятница
DayOfWeek.N.6=Суббота
DayOfWeek.N.7=Воскресенье

year_abbrevated=г.

Several.seconds.later=Через несколько секунд
Several.seconds.ago=Несколько секунд назад

Less.than.minute.later=Менее чем через минуту
Less.than.minute.ago=Меньше минуты назад

Hello.0=Здравствуйте, {0}!
_order.Items=Товары и услуги
Cart=Корзина
Added.n.items.to.cart=В корзину добавлено {0} товаров.
Item=Товар
Your.cart.is.empty=Ваша корзина пуста
Thanks.for.buy=Спасибо за покупку!
Thank.you.for.using.our.service=Благодарим вас за использование нашего сервиса.
Your.order.is.being.processed=Ваш заказ обрабатывается системой Suggest.io.
Return.to.my.ads=Назад к рекламным карточкам
Payment.for.order.N=Оплата заказа №{0}
Order.number=Номер заказа
Total.to.pay=Итого к оплате
Payment.method=Метод оплаты
Yandex.Kassa=Яндекс.Касса
Order.N=Заказ № {0}
Cost=Стоимость
Order.already.paid=Заказ уже оплачен
Pay.error=Ошибка при оплате
Go.to.payment.page=Перейти на страницу оплаты
Orders=Заказы
No.orders.found=Заказов не найдено.
Order.price=Стоимость заказа
Minimal.payment=Минимальный платёж
Order.0.is.paid=Заказ №{0} оплачен
We.received.payment.for.order.0=Поступила оплата по заказу №{0}.
Order.details=Подробности заказа
Paid=Оплачено

Radius=Радиус
in.radius.of.0.from.1=в {0} от {1}
in.radius.of.0=в радиусе {0}
n.km._kilometers={0} км
n.m._meters={0} м
n.cm._centimeters={0} см
in.geopoint.x=в точке {0}

Order.status.a=Корзина
Order.status.h=Ожидается подтверждение оплаты
Order.status.d=Оплачен
Order.status.s=Корзина суперпользователя

Item.status.a=Черновик
Item.status.b=Отправлено на модерацию
Item.status.c=Отклонёно
Item.status.f=Допущено к публикации
Item.status.o=Отображается сейчас
Item.status.z=Завершено


Item.type.a=На главной узла
Item.type.t=В геотеге на карте
Item.type.e=Внешнее размещение
Item.type.g=На карте
Item.type.m=На карте рекламных узлов
Item.type.d=В теге узла
Item.type.e=Пополнение баланса
Item.type.l=Захват геолокации

Moderation=Модерация
Picture.editing=Редактирование изображения
Moderation.needed=Требуется модерация

Refuse.mode.i=Только это размещение
Refuse.mode.req=Это и все неотмодерированные размещения
Refuse.mode.all=Все запрошенные и текущие размещения

Map=Карта
Map.settings.save.unsupported=Сохранено.
Adv.in.place=Размещение в месте

Domain.mode.sc=Отображение выдачи в домене

e.required.tags.or.main.screen=Требуется активировать размещение на главном экране или указать хотя бы один геотег.

Current.ad=Текущая карточка
Current.node=Текущий узел
x.on.advertisers.map={0} на карте рекламодателей
On.the.map=На карте
Does.not.adv.on.map=Не размещается на карте.
You.can.place.adn.node.on.map.below=Вы можете разместиться на карте рекламных узлов с помощью карты ниже. Другие пользователи системы смогут размещать рекламу в вашей выдаче.
Your.sc.will.be.opened.auto.when.user.geolocated.inside.circle=Ваша рекламная выдача будет автоматически отображаться пользователю, когда он находится внутри круга на карте.
Publish.node.on.adv.map=Размещение узла на карте рекламных узлов
Users.geo.location.capturing=Захват геолокации пользователей


Adv.for.free.without.moderation=Размещать бесплатно, без подтверждения?

price.RUB={0} ₽
price.USD=${0}
price.EUR=€{0}

currency.RUB=Рубль России
currency.USD=Доллар США
currency.EUR=Евро


Nodes.management=Управление узлами

_in.nodes.management=управления узлами
You.can.tweak.nodes.link.in.0.page=Вы можете настроить список узлов на странице {0}.

Beacon.name.example=Мой маячок
Identifier=Идентификатор
Example.id.0=Пример идентификатора: {0}
Type.new.name.for.beacon.0=Введите новое имя для маячка "{0}".
For.example.0=Например: {0}
Subnodes=Подчинённые узлы
N.nodes={0} узлов
N.disabled={0} выключенных
Adv.in.nodes=Размещение в узлах
Node.with.such.id.already.exists=Узел с таким id уже существует.
New.node=Новый узел
No.subnodes=Нет bluetooth-маячков и иных подчинённых узлов.

Coverage.area=Площадь покрытия
Ad.area.modules.count=Площадь карточки (кол-во модулей)
Unable.to.initialize.map=Невозможно инициализировать карту

Adv.geo.form.descr1=С помощью данной страницы, вы можете опубликовать вашу рекламную карточку в произвольной области на карте мира, в конкретных "узлах", отмеченных на карте маркерами с логотипами.
User.located.inside.are.or.location.will.see.your.offer=Пользователи, находящиеся в заданной географической области, или внутри выбранных локаций, увидят ваше предложение на экране устройства.
Good.to.know.that=Следует помнить, что:
Geo.circle.does.not.touch.any.nodes=Круг географической области размещения НЕ затрагивает географические области "узлов".
Nodes.can.contain.subnodes.sublocations.routers.tvs.beacons=Любой узел может содержать под-узлы, например: Bluetooth-маячки или их группы; внутренние помещения или объекты; интернет-сайты, привязанные к Suggest.io; WiFi-роутеры, телеэкраны и видео-проекторы с поддержкой Suggest.io, или иные рекламные носители. Стоимость размещения рекламы на каждом из таких под-узлов задаётся владельцем узла.
Adv.geo.form.descr.price=Стоимость размещения в области на карте зависит от площади круга. Стоимость размещения в конкретном объекте задаётся пользователем-владельцем этого объекта. Под географической картой находится таблица, где вы можете ознакомится с детализацией стоимости запрашиваемых услуг.

Support.request=Обращение в поддержку
Support.service=Служба поддержки
Or.contact.us.via.email=Или напишите нам

Company.about=О компании
OGRN=ОГРН
INN=ИНН
KPP=КПП
Organization.name=Наименование организации
_Cbca.name.full_=ООО «Умные Мальчики»
Legal.address=Юридический адрес
_Cbca.address.legal_=199178, г. Санкт-Петербург, проспект Малый В.О, д. 43/75 литера А пом. 10-Н
Mail.address=Почтовый адрес
_Cbca.address.mail_=199155, Санкт-Петербург, Морская наб. д.31/1, к.15
Checking.account=Расчётный счет
Bank=Банк
_Cbca.bank.name_=СЕВЕРО-ЗАПАДНЫЙ БАНК ПАО СБЕРБАНК
Bank.BIK=БИК банка
Correspondent.account=Корреспондентский счет

Upper.block=Верхний блок
also.displayed.in.grid=отображается в плитке
Delete.block=Удалить блок
Block=Блок
Content=Контент
Example.text=Пример текста
File.is.not.a.picture=Файл не является изображением
Apply=Применить
Cannot.checksum.file.0=Не удалётся рассчитать контрольную сумму файла {0}
Suggested.bg.colors=Предлагаемые цвета фона
Scale=Масштаб
Preparing=Подготовка
Uploading.file=Загрузка файла
Stretch.across=Растягивать по ширине

Main.block=Главный блок
Main.blocks.are.used.to.display.ad.inside.the.grid=Главные блоки используются для отображения данной карточки в плитке среди других карточек.
This.block.is.the.only.main=Текущий блок -- главный.
This.block.is.not.main=Текущий блок не является главным.
No.main.blocks.First.block.is.main=В карточке нет главных блоков. Верхний блок считается основным.
No.tags.here=Нет тегов в текущей локации
No.tags.found.for.1.query=Не найдено "{0}".

Go.to.node.ads=К рекламным карточкам узла
Personal.cabinet=Личный кабинет
Show.ad.opened=Отображать карточку развёрнутой?

On=Вкл
Off=Выкл

Search.start.typing=Я ищу...
N.selected={0} выбрано
Total=Итого
Lost.node=Потерянный узел
Next.node=Следующий узел
Previous.node=Предыдущий узел
To.beginning=В начало
To.end=В конец
Nothing.to.moderate=Нечего модерировать
No.incoming.adv.requests=Входящие запросы размещения отсутствуют.
Moderate.all.nodes=Модерировать все узлы
Moderate.requests.from.all.nodes=Модерировать размещения со всех узлов, а не только с текущего?
Go.to.moderation=Перейти к модерации
In.cart=В корзине
Adv.req.sent=Отправлен запрос размещения
Will.be.adv=Будет размещение

Layer=Слой
Layers=Слои
Uppermost=На самый верх
Downmost=В самый низ
Above=Выше
Below=Ниже
Go.into=Перейти
Location.changed=Местоположение изменилось
Go.back.to.0=Назад в {0}

Text.shadow=Тень текста
Shadow.color=Цвет тени

0.uses.geoloc.to.find.ads={0} использует геолокацию для обнаружения предложений рядом с вами.
0.uses.bt.to.find.ads.indoor={0} использует Bluetooth для обнаружения предложений внутри зданий и сооружений.
Allow.0=Разрешить {0}
Later=Позже
You.can.enable.0.later.on.left.panel=Вы можете активировать {0} позже на левой панели меню.
Settings.done.0.ready.for.using=Настройка завершена. {0} готов к началу работы.
_to.Finish=Завершить

Username=Имя пользователя
Not.my.pc=Чужой компьютер?

I.accept=Я принимаю
terms.of.service=условия использования сервиса
I.allow.personal.data.processing=Я разрешаю хранение и обработку моих персональных данных

Email.example=name@email.ru
Phone.number.example=+7 123 435 5433
Phone.or.email=Телефон или email
Mobile.phone.number=Номер мобильного
Code.from.sms=Код из смс-сообщения

Registration.code=Код регистрации: {0}
Sms.code.sent.to.your.phone.number.0=На указанный вами номер телефона {0} отправлено смс-сообщение с кодом подтверждения регистрации.

My.node=Мой узел
Type.previous.signup.data.to.start.password.recovery=Для сброса пароля, введите данные, указанные при регистрации ранее.

_Block.expand.w=Фон горизонтально
_Block.expand.f=Полностью
Expand=Растягивать
Dont.expand=Не растягивать

Line.height=Высота строк
Always.outlined=Постоянная обводка
Try.again=Попробовать ещё раз

Get.node.index.failed=Нет связи с узлом/сервером.
Http.response.unexpected=Нет связи с сервером.
Application=Приложение
Download.application=Скачать приложение
Node.ids.or.keys=ID узлов/ключи
Flag.value=Значение флага
Ordering=Порядок
Not.indexed.text=Неиндексируемый текст
Type.text...=Введите текст...
Original=Оригинал
File=Файл
Storage=Хранилище
Metadata=Метаданные
Picture=Картинка
Mime.type=Тип MIME
Media=Медиа
Open.file=Открыть файл
Go.to.node.0=Перейти к узлу {0}
Replace.node.ids=Перезаписать в id узлов
Append.to.node.ids=Дописать в конец id узлов
File.already.exist.on.node.0=Файл уже существует на узле {0}.
External.service=Внешний сервис
empty=пусто

Download.0=Скачать {0}

K._Kilo=К
M._Mega=М
G._Giga=Г
T._Tera=Т
Number.frac.delim=,
Number0.with.units1={0} {1}
Prefixed0.metric.unit1={0}{1}
B._Bytes=Б

Choose...=Выберите...
Operating.system.family=Семейство операционных систем
No.downloads.available=Нет доступных загрузок
Malfunction=Неисправность
Open.0=Открыть {0}
Hide=Скрыть
Settings=Настройки
0.not.supported.on.this.platform.1={0} не поддерживается на текущей платформе ({1}).
Install.app.for.access.to.0=Установите приложение для получения доступа к {0}.

Notifications=Уведомления
Notify.about.offers.nearby=Уведомлять о предложениях поблизости.

0.offers.nearby={0} предложений рядом с вами
One.offer.nearby=Есть предложение рядом!

0._inDistance.1={0} ({1})
Show.offers.0=Показать предложения {0}

Background.mode=Фоновый режим
Background.monitoring.offers.nearby=Уведомление о новых предложениях рядом с вами. Вы можете настроить мониторинг в настройках приложения.
Daemon.toast.title=Фоновый мониторинг
Daemon.toast.descr=Обнаружение новых предложений поблизости.
Unsupported.browser.or.fatal.failure=Неподдерживаемый браузер или фатальная ошибка системы.

Not.connected=Нет соединения.
Retry=Повторить
Recents=Недавние

_to.Register._thing=Зарегистрировать
Add.beacon.to.account=Присоединить этот маячок к учётной записи.
To.control.beacons.need.login=Войдите в систему для доступа к функциями управления маячками.
Parent.node=Родительский узел

Logout.account=Выйти из учетной записи
Are.you.sure.to.logout.account=Вы уверены, что хотите выйти из учётной записи?

Mode=Режим
Ad.adv.manage=Размещение карточки

Bg.location=Текущее местоположение
Bg.location.hint=Обнаружение предложений поблизости

Outline=Обводка
Transparent=Прозрачно
Define.manually=Задать вручную

