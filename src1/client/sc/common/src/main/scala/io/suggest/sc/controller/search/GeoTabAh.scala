...
import io.suggest.sc.model.inx.{MScIndex, MScSideBars, MapReIndex, SideBarOpenClose}
import io.suggest.sc.model.{HandleScApiResp, MScRoot}
      ...
      (for {
        ...
        if (mnode.ntype.exists(_ eqOrHasParent MNodeTypes.Tag)) {
          ...
          val fx = Effect.action {
            MapReIndex(
              mnode.nodeId,
              scGeoUtil.nodePropsShapesToNodeGeoPoint( nodePropsShapes, v0 ),
            )
          }
