...
import io.suggest.geo.{DistanceUtil, IDistanceFrom, IDistanceUtilJs, ILPolygonGs}
  ...
  class Backend($: BackendScope[Props, Props_t]) {
    ...
    def render(propsProxy: Props): VdomElement = {
      ...
      val distanceMOpt = for {
        ...
        // Initialize distance util, if any.
        distanceUtilJs <- distanceUtilJsOpt
        locLl <- distanceUtilJs.prepareDistanceTo( distanceToPoint )
        ...
        distancesIter = {
          (
            ...
            (for {
              ...
              distanceFrom <- nodeShape
                .centerPoint
                .flatMap { distanceUtilJs.prepareDistanceTo }
                .orElse {
                  nodeShape match {
                    case poly: ILPolygonGs =>
                      distanceUtilJs.geoCenterOfPolygon( poly )
                    case _ =>
                      None
                  }
                }
                .orElse {
                  ...
                  distanceUtilJs.prepareDistanceTo( nodeShape.firstPoint )
                }
            } yield {
              distanceFrom
            }) #::
