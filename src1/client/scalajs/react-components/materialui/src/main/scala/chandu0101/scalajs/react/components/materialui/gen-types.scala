...
class AutoComplete(val value: String) extends AnyVal
object AutoComplete {
  val on = new AutoComplete("on")
  val off = new AutoComplete("off")
  val values = List(on, off)
}
...
class FormMethodType(val value: String) extends AnyVal
object FormMethodType {
  val get = new FormMethodType("get")
  val post = new FormMethodType("post")
  val values = List(get, post)
}


class FormTarget(val value: String) extends AnyVal
object FormTarget {
  val _blank = new FormTarget("_blank")
  val _self = new FormTarget("_self")
  val _parent = new FormTarget("_parent")
  val _top = new FormTarget("_top")
  val framename = new FormTarget("framename")
  val values = List(_blank, _self, _parent, _top, framename)
}
...
class InputEncodingType(val value: String) extends AnyVal
object InputEncodingType {
  val application_x_www_form_urlencoded = new InputEncodingType("application/x-www-form-urlencoded")
  val multipart_form_data = new InputEncodingType("multipart/form-data")
  val text_plain = new InputEncodingType("text/plain")
  val values = List(application_x_www_form_urlencoded, multipart_form_data, text_plain)
}
