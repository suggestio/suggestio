...
import io.suggest.ble.BluetoothAdvertisementData
import io.suggest.ble.BluetoothAdvertisementData.ScanRecordToken
...
/**
  ...
  */
object CdvEddyStoneParser extends BeaconParserFactory {
  override type T = CdvEddyStoneParser
}
...
/** Parser for beacons according to EddyStone-UID specs.
  ...
  */
case class CdvEddyStoneParser(override val dev: BtDevice)
  extends CdvBeaconParser
  with Log
{
        ...
        if ( rawType ==* JsTypes.OBJECT ) {
          ...
          val resps = advDataRaw match {
            ...
            case arrBuf: ArrayBuffer =>
              val parsedTokens = BluetoothAdvertisementData.parseScanRecord( new Uint8Array(arrBuf) )
              for {
            ...
            case _ =>
              ...
              for {
                ...
                svcUuidTok = ScanRecordToken.UuidToken( svcUuidPart, BluetoothAdvertisementData.UuidHelper.B16 )
