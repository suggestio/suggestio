...
class GoogleLoginSpec extends PlaySpecification with Mockito {
  ...
  class TestEnvironment(_routes: => RoutesService = new RoutesService.Default(),
      ...
      _httpService: => HttpService = new HttpService.Default()) extends RuntimeEnvironment.Default[DemoUser] {
  ...
  val googleConfig = Map(
    "smtp.mock" -> true, "application.secret" -> "secret", "application.router" -> "securesocial.Routes", "securesocial.google.clientId" -> "clientid", "securesocial.google.clientSecret" -> "clientsecret", "securesocial.google.authorizationUrl" -> "authorizationUrl", "securesocial.google.accessTokenUrl" -> "accessTokenUrl", "securesocial.google.scope" -> "scope"
  )
