...
trait SecureSocial[U] extends Controller {
  ...
  trait ISecuredRequest[A] extends Request[A] {
    def user: U
   ...
   */
  case class SecuredRequest[A](user: U, authenticator: Authenticator[U], request: Request[A])
    extends WrappedRequest(request)
    with ISecuredRequest[A]

  trait IRequestWithUser[A] extends Request[A] {
    def user: Option[U]
    def authenticator: Option[Authenticator[U]]
  }
  /**
   * A request that adds the User for the current call
   */
  case class RequestWithUser[A](user: Option[U], authenticator: Option[Authenticator[U]], request: Request[A])
    extends WrappedRequest(request)
    with IRequestWithUser[A]
   ...
   */
  object UserAwareAction extends UserAwareActionBuilderBase

  /**
   * The UserAwareAction builder
   */
  trait UserAwareActionBuilderBase extends ActionBuilder[RequestWithUser] {
