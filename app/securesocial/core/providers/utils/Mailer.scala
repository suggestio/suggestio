 ...
 */
trait Mailer {
  def sendAlreadyRegisteredEmail(user: IProfile)(implicit request: RequestHeader, lang: Lang)
  def sendSignUpEmail(to: String, token: String)(implicit request: RequestHeader, lang: Lang)
  def sendWelcomeEmail(user: IProfile)(implicit request: RequestHeader, lang: Lang)
  def sendPasswordResetEmail(user: IProfile, token: String)(implicit request: RequestHeader, lang: Lang)
  def sendUnkownEmailNotice(email: String)(implicit request: RequestHeader, lang: Lang)
  def sendPasswordChangedNotice(user: IProfile)(implicit request: RequestHeader, lang: Lang)
...
object Mailer {
  ...
  class Default(mailTemplates: MailTemplates) extends Mailer {
    ...
    override def sendAlreadyRegisteredEmail(user: IProfile)(implicit request: RequestHeader, lang: Lang) {
    ...
    override def sendWelcomeEmail(user: IProfile)(implicit request: RequestHeader, lang: Lang) {
      val txtAndHtml = mailTemplates.getWelcomeEmail(user)
      sendEmail(Messages(WelcomeEmailSubject), user.email.get, txtAndHtml)

    }

    override def sendPasswordResetEmail(user: IProfile, token: String)(implicit request: RequestHeader, lang: Lang) {
    ...
    override def sendPasswordChangedNotice(user: IProfile)(implicit request: RequestHeader, lang: Lang) {
