...
object MBillMmpDaily extends FindByContract[MBillMmpDaily] {
  ...
  val rowParser = get[Pk[Int]]("id") ~ get[Int]("contract_id") ~ get[Float]("mmp_weekday") ~ get[Float]("mmp_weekend") ~
    ...
    get[String]("weekend_cal_id") ~ get[String]("prime_cal_id") map {
    case id ~ contractId ~ mmpWeekday ~ mmpWeekend ~ mmpPrimetime ~ currencyCode ~ onStartPage ~ weekendCalId ~ primeCalId =>
      MBillMmpDaily(
        ...
        onStartPage = onStartPage, weekendCalId = weekendCalId, primeCalId = primeCalId
      )
  }
...
case class MBillMmpDaily(
  ...
  override def saveInsert(implicit c: Connection): MBillMmpDaily = {
    SQL("INSERT INTO " + TABLE_NAME + "(contract_id, currency_code, mmp_weekday, mmp_weekend, mmp_primetime, on_start_page, weekend_cal_id, prime_cal_id) " +
      "VALUES ({contractId}, {currencyCode}, {mmpWeekday}, {mmpWeekend}, {mmpPrimetime}, {onStartPage}, {weekendCalId}, {primeCalId})")
  ...
  override def saveUpdate(implicit c: Connection): Int = {
    SQL("UPDATE " + TABLE_NAME + " SET currency_code = {currencyCode}, mmp_weekday = {mmpWeekday}, " +
      "mmp_weekend = {mmpWeekend}, mmp_primetime = {mmpPrimetime}, on_start_page = {onStartPage}," +
