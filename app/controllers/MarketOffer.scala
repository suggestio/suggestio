...
object MarketOffer extends SioController with MacroLogsImpl {
  ...
  private def vendorM = "vendor" -> nonEmptyText(maxLength = YmlSax.VENDOR_MAXLEN)
  ...
  private def modelM  = "model"  -> nonEmptyText(maxLength = YmlSax.OFFER_MODEL_MAXLEN)

  /** Маппер списка доступных цветов (чекбоксы) разбирается и причёсывается тут. */
  private def colorsM = "colors" -> list(text(maxLength = 16))
    .transform(
  ...
  private def sizesM = "sizes" -> text(maxLength = 64)
    .transform(
