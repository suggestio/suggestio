...
object BlocksConfPreStripUtil extends PlayMacroLogsImpl {
  ...
  /**
   * Перевыставить значение mad.blockMeta.width у всех карточек.
   * @return Фьючерс с кол-вом обновленных карточек.
   */
  def resetAllMadBmWidth(): Future[Int] = {
    warn("resetAllMadBmWidth(): Starting...")
    updateAllBlockMeta { bm0 =>
      bm0.copy(
        width = BlocksConf(bm0.blockId).blockWidth
      )
    }
  }
...
class BlocksConfPreStripUtilJmx extends JMXBase with BlocksConfPreStripUtilJmxMBean {
  ...
  override def resetAllMadBmWidth(): String = {
    val resFut = BlocksConfPreStripUtil.resetAllMadBmWidth()
