...
class YmCatTranslatorTest extends FlatSpec with Matchers {
  ...
  "Translator" should "handle 1-level categories from carrida.ru yml price" in {
    ...
    List(
      CAT_TREE / "Авто, мото" / "Аксессуары" / "Универсальные зарядные устройства и инверторы",
      CAT_TREE / "Авто, мото" / "Запчасти" / "Зарядные устройства для аккумуляторов"
    ...
    //r("446931")  shouldBe  CAT_TREE / "Авто, мото" / "Аудио- и видеотехника" / "Усилители"    // TODO Есть проблемы: улучшалка, учитывающая порядок токенов, не особо-то работает.
    //r("449028")  shouldBe  CAT_TREE / "Авто, мото" / "Аудио- и видеотехника" / "Телевизоры и мониторы"  // TODO Есть проблемы - выпадают тут "автомобильные видеорегистраторы"
