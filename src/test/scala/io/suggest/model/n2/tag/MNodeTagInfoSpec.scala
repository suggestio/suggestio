...
class MNodeTagInfoSpec extends FlatSpec {
  ...
  private def t(mnti: MNodeExtras): Unit = {
    val jsVal = Json.toJson(mnti)
    val mapped = jsVal.validate[MNodeExtras]
    assert(mapped.isSuccess, mapped)
    mapped.get  shouldBe  mnti
  }

  classOf[MNodeExtras].getSimpleName  should  "support EMPTY JSON serialize/deserialize" in {
    t( MNodeExtras.empty )
  }
