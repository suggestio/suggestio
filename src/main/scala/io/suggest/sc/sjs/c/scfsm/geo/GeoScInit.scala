...
/**
  ...
  * Description: Инициализатор гео-выдачи второго поколения.
  *
  * Первое было реализовано через цепочку состояний, но уже на раннем этапе состояния стали переусложненными,
  * и их стало слишком много для столь стрёмного и частого одного действия.
  *
  * Было решено использовать для этого всего цепочки из Future.
...
trait GeoScInit extends Index { me =>
      ...
      with ProcessIndexReceivedUtil
  {
    ...
    protected[this] def _geoScInit(): Unit = {
